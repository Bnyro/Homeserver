const apiURL = 'http://alpine.fritz.box:443';
let calendar;
...
const removeEvent = async (event) => {
    styledConfirm(
        ...
        'Delete "' + event.title + '"?',
        async function() {
            await fetch(apiURL + '/events/delete', {
                method: 'DELETE',
                headers: {
...
$('#create>form').addEventListener('submit', async () => {
    ...
    const event = {
        id: Date.now(),
        title: $('#title').value,
        start: $('#startdate').value + 'T' + $('#starttime').value + ':00+' + offset + ':00',
        end: $('#enddate').value + 'T' + $('#endtime').value + ':00+' + offset + ':00',
        color: $('#color').value
    };
    await fetch(apiURL + '/events/post', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
